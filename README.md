...
│                      用户端入口层                          │
├────────────────┬─────────────────┬──────────────────────┤
│ 管理后台 Web    │ Android 移动端   │ RAG 前端 Web          │
│ (React)        │ (Native Java)   │ (React + Zustand)    │
└────────┬───────┴────────┬────────┴──────────┬───────────┘
         │                │                    │
         └────────────────┴────────────────────┘
                          │
...
└─────────┘         └──────────┘         └──────────┘
    │                     │                     │
┌─────────┐         ┌──────────┐         ┌──────────┐
│ comment │         │ relation │         │  record  │
│ 评论服务 │         │ 关系服务  │         │ 记录服务  │
└─────────┘         └──────────┘         └──────────┘
    │                     │                     │
┌─────────┐         ┌──────────┐         ┌──────────┐
│ insight │         │   oss    │         │ RocketMQ │
│ 洞察服务 │         │ 文件存储  │         │ 消息队列  │
└─────────┘         └──────────┘         └──────────┘
                          │
         ┌────────────────┴────────────────────┐
...
| 服务名 | 端口 | 职责 |
|--------|------|------|
| **athena-gateway** | 9090 | API 网关、统一鉴权、路由转发 |
| **athena-userauth** | 9091 | 用户认证、用户资料管理 |
| **athena-ground** | 9092 | 内容发布、审核、广场管理 |
| **athena-comment** | 9093 | 评论、回复、点赞 |
| **athena-relation** | 9094 | 关注、粉丝关系 |
| **athena-record** | 9095 | 经期记录、健康数据 |
| **athena-insight** | 9096 | 推荐算法、分析报告 |
| **athena-oss** | 9097 | 文件上传、对象存储 |
| **athena-rag** | 9098 | RAG 问答、知识库、向量检索 |
