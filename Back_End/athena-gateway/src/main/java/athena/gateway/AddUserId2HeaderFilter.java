...
import static athena.athenaframework.utils.RedisConstants.SA_TOKEN_TOKEN_KEY_PREFIX;
...
public class AddUserId2HeaderFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 构建 Redis Key
        String tokenRedisKey = SA_TOKEN_TOKEN_KEY_PREFIX + token;
        // 查询 Redis, 获取用户 ID
        String userIdstring = stringRedisTemplate.opsForValue().get(tokenRedisKey);

        if (Objects.isNull(userIdstring) ||userIdstring.trim().isEmpty()) {
            log.info("token无效，无对应用户");
            // 若没有登录，则直接放行
            return chain.filter(exchange);
        }
