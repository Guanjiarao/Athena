...
每个子问题会拆出 KB 意图和 MCP 意图。当前代码里 MCP 执行被注释禁用，所以实际有效的是 KB 检索：
...
如果检索结果为空，pipeline 不调用 LLM，直接返回固定提示：
...
2. 把 retrieved chunks 设置到 callback，后续 finish 事件会从这里生成引用。
...
- chunk 必须有 metadata。
- chunk score 必须大于等于 `searchChannelProperties.noteReference.minScoreThreshold`。
...
前端调用 `POST /rag/v3/stop?taskId=...` 后：
...
实际取消逻辑在 `StreamTaskManager`，本文未展开其内部实现。已确认的是：

- `StreamChatEventHandler` 初始化时注册 task。
- pipeline 拿到 `StreamCancellationHandle` 后绑定到 task。
- `onContent`、`onThinking`、`onComplete` 都会先检查 `taskManager.isCancelled(taskId)`。
...
说明当前系统不仅返回 SSE，也会记录 RAG 运行链路，用于后续 trace 查询和排障。
...
| 模块 | 文件 |
|---|---|
| HTTP/SSE 入口 | `bootstrap/src/main/java/com/nageoffer/ai/ragent/rag/controller/RAGChatController.java` |
| Chat 服务入口 | `bootstrap/src/main/java/com/nageoffer/ai/ragent/rag/service/impl/RAGChatServiceImpl.java` |
| 主业务 pipeline | `bootstrap/src/main/java/com/nageoffer/ai/ragent/rag/service/pipeline/StreamChatPipeline.java` |
| 查询改写/拆分 | `bootstrap/src/main/java/com/nageoffer/ai/ragent/rag/core/rewrite/MultiQuestionRewriteService.java` |
| 意图解析 | `bootstrap/src/main/java/com/nageoffer/ai/ragent/rag/core/intent/IntentResolver.java` |
| 检索编排 | `bootstrap/src/main/java/com/nageoffer/ai/ragent/rag/core/retrieve/RetrievalEngine.java` |
...
| LLM 接口 | `infra-ai/src/main/java/com/nageoffer/ai/ragent/infra/chat/LLMService.java` |
| SSE callback 工厂 | `bootstrap/src/main/java/com/nageoffer/ai/ragent/rag/service/handler/StreamCallbackFactory.java` |
| SSE 事件处理 | `bootstrap/src/main/java/com/nageoffer/ai/ragent/rag/service/handler/StreamChatEventHandler.java` |
| system-only prompt | `bootstrap/src/main/resources/prompt/answer-chat-system.st` |
