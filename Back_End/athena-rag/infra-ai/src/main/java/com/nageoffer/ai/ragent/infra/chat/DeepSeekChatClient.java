...
public class DeepSeekChatClient implements ChatClient {
    ...
    public String chat(ChatRequest request, ModelTarget target) {
        ...
        try (Response response = httpClient.newCall(requestHttp).execute()) {
            ...
            throw new ModelClientException("DeepSeek 同步请求失败: " + e.getMessage(),
    ...
    private JsonObject buildRequestBody(ChatRequest request, ModelTarget target, boolean stream) {
        ...
        reqBody.addProperty("stream", stream);
        ...
        thinking.addProperty("type", thinkingEnabled ? THINKING_TYPE_ENABLED : THINKING_TYPE_DISABLED);
        reqBody.add("thinking", thinking);

        if (thinkingEnabled) {
            reqBody.addProperty("reasoning_effort", DEFAULT_REASONING_EFFORT);
        } else {
