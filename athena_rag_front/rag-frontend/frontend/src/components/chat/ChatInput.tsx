...
export function ChatInput() {
  ...
  return (
    <div className="space-y-4">
      <div
        className={cn(
          "relative flex flex-col rounded-2xl border bg-honey-50/95 backdrop-blur px-4 pt-3 pb-2 transition-all duration-200",
          isFocused
            ? "border-honey-400 shadow-warm ring-2 ring-honey-300/20"
            : "border-honey-200 hover:border-honey-300"
        )}
      >
        <div className="relative">
          <Textarea
            ...
            aria-label="聊天输入框"
          />
          <div className="pointer-events-none absolute bottom-0 left-0 right-0 h-[10px] bg-gradient-to-b from-honey-50/0 via-honey-50/40 to-honey-50/95" />
        </div>
        <div className="relative mt-2 flex items-center">
          <button
            ...
            className={cn(
              ...
              deepThinkingEnabled
                ? "border-honey-400 bg-honey-200 text-warm-800"
      ...
      <p className="text-center text-xs text-warm-500">
        <kbd className="rounded bg-white/80 px-1.5 py-0.5 text-warm-700 border border-honey-200">Enter</kbd> 发送
        <span className="px-1.5">·</span>
        <kbd className="rounded bg-white/80 px-1.5 py-0.5 text-warm-700 border border-honey-200">
          Shift + Enter
        </kbd>{" "}
        换行
