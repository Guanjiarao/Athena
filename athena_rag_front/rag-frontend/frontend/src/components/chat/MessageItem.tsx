    ...
    <div className="group flex">
      <div className="min-w-0 flex-1 space-y-4">
        ...
        {!isThinking && hasThinking ? (
          <div className="overflow-hidden rounded-lg border border-honey-300 bg-honey-100">
            <button
              type="button"
              onClick={() => setThinkingExpanded((prev) => !prev)}
              className="flex w-full items-center gap-2 px-4 py-3 text-left transition-colors hover:bg-honey-200/50"
            >
              <div className="flex flex-1 items-center gap-2">
                <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-honey-200">
                  <Brain className="h-4 w-4 text-honey-700" />
                </div>
                <span className="text-sm font-medium text-honey-800">深度思考</span>
                {thinkingDuration ? (
                  <span className="rounded-full bg-honey-200 px-2 py-0.5 text-xs text-honey-700">
                    {thinkingDuration}
                  </span>
                ) : null}
              </div>
              <ChevronDown
                className={cn(
                  "h-4 w-4 text-honey-600 transition-transform",
                  thinkingExpanded && "rotate-180"
                )}
              />
            </button>
            {thinkingExpanded ? (
              <div className="border-t border-honey-300 px-4 pb-4">
                <div className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-warm-800">
                  {message.thinking}
                </div>
              </div>
            ) : null}
