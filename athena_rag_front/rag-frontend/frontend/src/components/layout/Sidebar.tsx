        ...
        <div className="border-b border-[#F0F0F0] pb-3">
          <div className="flex items-center gap-3">
            ...
            <div style={{ fontFamily: sessionTitleFont }}>
              <p className="text-base font-semibold text-warm-900">Ragent AI 智能体</p>
              <p className="text-xs text-warm-600">Powered by AI</p>
            </div>
          </div>
        </div>
        <div className="py-3 space-y-4">
          <div className="relative overflow-hidden rounded-2xl border border-honey-200 bg-gradient-to-br from-honey-50 via-white to-warm-100 p-3 shadow-sm">
            <span
              aria-hidden="true"
              className="absolute -right-10 -top-10 h-24 w-24 rounded-full bg-honey-300/50 blur-2xl"
            />
            <span
              aria-hidden="true"
              className="absolute -left-12 -bottom-10 h-28 w-28 rounded-full bg-warm-300/50 blur-2xl"
            />
            <div className="relative">
              <div className="flex items-center justify-between px-1">
                <span className="text-[11px] font-semibold text-warm-600">快速开始</span>
                <span className="rounded-full bg-white/80 px-2 py-0.5 text-[10px] font-semibold text-honey-700">
                  新内容
                </span>
              </div>
              <button
              ...
              >
                <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-sunset text-white shadow-warm">
                  <Plus className="h-4 w-4" />
                </span>
                <span className="flex-1">
                  <span className="block text-sm font-semibold text-warm-900">新建对话</span>
                  <span className="block text-xs text-warm-600">从空白开始</span>
                </span>
              </button>
              {user?.role === "admin" ? (
                <button
                  type="button"
                  className="mt-2 inline-flex items-center gap-2 rounded-full border border-honey-300 bg-honey-100 px-3 py-1.5 text-xs font-semibold text-honey-700 transition-colors hover:bg-honey-200"
                  onClick={() => {
        ...
        <div className="relative flex-1 min-h-0">
          <div className="h-full overflow-y-auto sidebar-scroll">
            {sessions.length === 0 && (!sessionsLoaded || isLoading) ? (
              <div
                className="flex h-full items-center justify-center text-warm-600"
              ...
              <div
                className="flex h-full flex-col items-center justify-center text-warm-600"
                style={{ fontFamily: sessionTitleFont }}
              >
              ...
              <div>
                {groupedSessions.map((group, index) => (
                  <div key={group.label} className={cn("flex flex-col", index === 0 ? "mt-0" : "mt-4")}>
                    <p className="mb-1.5 pl-3 text-[12px] font-normal leading-[18px] text-warm-600">
                      {group.label}
                    </p>
                      ...
                      <div
                        ...
                        className={cn(
                          ...
                          currentSessionId === session.id
                            ? "bg-honey-200/60 text-honey-800"
                            : "text-warm-900 hover:bg-honey-100/50"
                        )}
                        role="button"
                        tabIndex={0}
                        onClick={() => {
                      ...
                      >
                        {renamingId === session.id ? (
                          <input
                            ...
                            className="h-6 flex-1 rounded-md border border-honey-300 bg-white px-2 text-[14px] leading-[22px] text-warm-900 focus:border-honey-500 focus:outline-none"
                          />
                        ) : (
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <button
                              type="button"
                              className={cn(
                                "flex h-6 w-6 items-center justify-center rounded text-warm-600 transition-opacity duration-150 hover:bg-honey-200",
                                currentSessionId === session.id
                                  ? "pointer-events-auto opacity-100 text-honey-700"
