...
import { ChevronDown, FileSearch, LogOut, Rocket, Trash2, User } from 'lucide-react'
...
function UserMenu() {
  ...
  const displayName =
    user?.role === 'admin' ? '管理员' : user?.role === 'kol' ? '发布者' : '用户'
        ...
        <nav className="flex flex-col gap-0.5 px-3 pb-6">
          {user?.role === 'admin' ? (
            <>
              ...
              <NavLink to="/content-manage" className={sidebarNavLinkClass}>
                <Trash2 className="size-5 shrink-0 text-zinc-800" aria-hidden />
                内容管理
              </NavLink>
            </>
          ) : null}
