...
export function AuditList() {
  ...
  const openRejectDialog = useCallback(() => {
    if (!activeNoteId || deciding || detailLoading) return
    setDetailError(null)
    setRejectReasonError(null)
    setRejectDialogOpen(true)
  }, [activeNoteId, deciding, detailLoading])
  ...
  const handleApprove = useCallback(async () => {
    ...
    setDetailError(null)
    try {
      const resp = await approveNote(activeNoteId)
  ...
  const submitReject = useCallback(async () => {
    ...
    setRejectReasonError(null)
    try {
      const resp = await rejectNote({
        noteId: activeNoteId,
        reviewRemark: trimmedReason,
      })
      if (resp.data?.code === 200) {
        setToast('已驳回')
        closeRejectDialog()
      ...
      setRejectReasonError(resp.data?.message || '操作失败，请稍后重试')
    } catch (e) {
      if (isAxiosError(e)) {
        const msg =
          (e.response?.data as { message?: string } | undefined)?.message ||
          e.message
        setRejectReasonError(msg || '操作失败，请稍后重试')
      } else {
        setRejectReasonError('操作失败，请稍后重试')
      }
    } finally {
      setDeciding(false)
    }
  }, [activeNoteId, closeDrawer, closeRejectDialog, loadPending, rejectReason])
  ...
  }, [error, loading, openReview, rows])

  const detailImgUrls = parseImgUrls(detail?.imgUrls)
  const detailCoverUrl =
    detail?.coverUrl ||
    rows.find((row) => row.blogId === activeNoteId)?.coverUrl ||
    null
      ...
      {rejectDialogOpen ? (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <button
            ...
            aria-label="关闭驳回理由弹窗"
          />
          <div className="relative z-10 w-full max-w-lg rounded-2xl border border-zinc-200 bg-white shadow-2xl">
            <div className="border-b border-zinc-200 px-6 py-4">
              <h3 className="text-lg font-semibold text-zinc-900">填写驳回理由</h3>
              <p className="mt-1 text-sm text-zinc-500">
                请填写具体的驳回原因，提交后会作为审核备注保存。
              </p>
            </div>
            ...
            <div className="space-y-3 px-6 py-5">
              ...
              <textarea
                ...
                placeholder="例如：标题与正文不符，封面质量不足，请修改后重新提交。"
                className="min-h-32 w-full resize-y rounded-xl border border-zinc-200 px-3 py-2.5 text-sm text-zinc-900 outline-none transition focus:border-zinc-400 focus:ring-2 focus:ring-zinc-200"
              />
              {rejectReasonError ? (
                <p className="text-sm text-red-500">{rejectReasonError}</p>
              ) : (
                <p className="text-sm text-zinc-500">
                  发布者后续查看审核结果时，会使用这段驳回备注。
                </p>
              )}
            </div>
