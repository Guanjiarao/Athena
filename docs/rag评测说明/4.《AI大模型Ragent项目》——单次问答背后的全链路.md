...
召回的 chunk 文本（与 chunkIds 顺序对应）

retrievedContextDocIds

chunk 维度的业务 docId（与 contexts 一一对应，保留 null，不去重）

intentLeafIds

每个子问题 top-1 的意图叶子节点 ID
...
[PROD_PHONE_001, PROD_PHONE_001, POLICY_RETURN_002, PROD_PHONE_003, null]

5（跟 chunk 数一样）

retrievedDocIds

[PROD_PHONE_001, POLICY_RETURN_002, PROD_PHONE_003]
...
取 conversationId、taskId

message（type=response）

答案增量 delta

拼接 response 文本 + TTFT 打点
...
final_status 记为 success

reject

拒绝回答

final_status 记为 refused，记录原因

cancel

用户取消

final_status 记为 cancelled
...
reference_doc_ids / reference_doc_ids_nice
...
success / refused / error / cancelled / unknown
...
ThreadPoolExecutor 并行调度，写文件用锁保护

赶时间跑全量，需要确认 ragent 能扛住并发
...
SSE 流读取超时（长答案生成可能要几十秒）

EVAL_CONNECT_TIMEOUT
...
这不是 bug，是有意识的工程权衡。如果要消除这两个问题，就得改造生产链路——在 SSE 流里夹带检索中间产物，或者在生产代码里加一层旁路日志落库。代价是侵入生产代码、增加维护负担。当前做法的好处是生产链路完全不改，TTFT 和 response 是真实数据，检索证据足够近似。
