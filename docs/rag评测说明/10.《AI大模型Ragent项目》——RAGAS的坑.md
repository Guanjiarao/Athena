...
NaN 样本逐条重试（超时放宽到 1200s）

全量跑完后仍有 NaN

5

RunConfig(max_retries=2) 每次 LLM 调用内部重试
...
省 2/3 的调用量，开发阶段看趋势够用

复用 runs 文件

python -m eval rag score

不重跑 runner，只重算指标。改 judge 配置后复用同一份 runs，不额外调被评接口
...
所有 answer_correctness < 0.6 的样本

分数异常波动

跟上次相比下降 > 5% 的指标，看低分样本是真退化还是误判

高风险意图

售后政策、保修条款、价格类样本，不管分数高低都抽查

首次跑 RAGAS

全量抽查一遍，建立对 judge 打分准确度的基线认知
...
用 curl 或代码发一次 chat completions 请求
...
对比 JUDGE_MODEL 和 Ragent 配置的 chat 模型
...
查 eval_set_v1.jsonl 的 ground_truth 字段非空

answer_correctness / context_recall 算不了

6

runs 里 contexts 非空

检查 JSONL 里 retrieved_contexts 非空率
