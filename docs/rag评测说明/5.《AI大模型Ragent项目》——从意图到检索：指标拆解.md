...
正确文档被召回了，但相关内容不在这些 chunk 里——分块策略需要调整
...
别的文档的 chunk 碰巧覆盖了部分信息——检索没对齐，答案靠运气
...
自建指标的优势在于快和确定：秒级出结果，跑 100 次结果一样，适合挂 CI 当闸门。RAGAS 的优势在于语义级：能发现文档找到了但 chunk 里没有关键信息这类自建指标看不出来的问题。两套配合着看，检索阶段的问题基本能够发现。
