...
> 版本：v1.5  
> 更新时间：2026-06-10  
> 适用范围：管理员网站端 `admin_web_front`、Android 用户端 `athena_app_front`、RAG 前端 `athena_rag_front`、后端微服务体系

---

## 1. 文档定位与阅读逻辑

本文档用于说明 Athena 系统的总体技术架构和多端协同关系。文档按”整体架构 -> 端侧架构 -> 后端支撑 -> 业务链路 -> 详细实现”的顺序组织，使读者能够先理解系统全貌，再进入各端实现细节。

阅读顺序建议如下：

1. 先阅读第 2 章，了解系统由哪些端、哪些后端服务和哪些基础设施组成。
2. 再阅读第 3 章，理解管理员网站端、Android 用户端、RAG 前端与后端服务之间的职责分工。
3. 然后阅读第 4 章，掌握登录鉴权、内容、AI、健康记录、上传等关键链路如何跨端流转。
4. 需要了解后端微服务内部结构时，阅读第 6 章。该章节保留原后端架构内容，技术细节不做改写。
5. 需要了解三个前端工程内部结构时，阅读第 7、8、9 章。
...
- **[系统总体架构](#2-系统总体架构)** — 整体架构视图、分层说明、设计原则
- **[前后端职责划分](#3-前后端职责划分)** — 三端与后端服务的职责边界
- **[业务链路](#4-核心业务链路与跨端协同)** — 登录鉴权、内容流转、AI 对话等关键链路
- **[后端微服务](#6-后端微服务详细架构)** — 微服务架构、网关、数据库、消息队列
- **[管理员网站端](#7-管理员网站端详细架构)** `admin_web_front` — React 技术栈与目录结构
- **[Android 用户端](#8-android-用户端详细架构)** `athena_app_front` — Native Android 架构
- **[RAG 前端](#10-rag-前端架构)** `athena_rag_front` — React + Zustand 状态管理

---

## 2. 系统总体架构

Athena 是一个面向女性健康场景的多前端、多后端服务系统。整体由管理员网站端、Android 用户端、RAG 前端、API 网关、后端微服务、AI/RAG 服务、对象存储、数据库和缓存等部分组成。

### 2.1 总体架构视图

![](D:\athenaworktwo\athena\docs\attachments\Athena_Architecture.png)

### 2.2 架构分层说明

| 层级 | 组成 | 核心职责 |
| --- | --- | --- |
| 用户入口层 | 管理员网站端、Android 用户端、RAG 前端 | 承载不同角色的交互入口和端侧体验 |
| 前端应用层 | React SPA、Android Activity/Fragment、RAG React SPA | 页面组织、路由导航、状态展示、输入校验、调用接口 |
| 接入层 | Spring Cloud Gateway | 统一 API 入口、Token 校验、路由转发、用户上下文透传 |
| 业务服务层 | userauth、ground、comment、relation、record、insight、oss、rag | 分域承载登录、内容、互动、健康记录、AI、知识库等业务能力 |
...
RAG 前端 (`athena_rag_front/rag-frontend/frontend`) 是 Athena 的智能问答知识库管理系统前端应用，与后端 `athena-rag` 服务配合，提供完整的 RAG 系统前端能力。
...
与双端系统的关系：
- RAG 前端独立于 `admin_web_front` 和 `athena_app_front`
- 专注于 RAG 场景，不承载内容审核、社区互动等业务
- 可作为独立服务部署，也可与其他前端集成
...
| | Vite | 5.4.3 | 构建工具 |
| **路由** | React Router | 6.26.2 | 前端路由 |
| **状态管理** | Zustand | 4.5.5 | 全局状态 |
| **UI 组件** | Radix UI | - | 无障碍组件 |
| | shadcn/ui | - | 组件设计系统 |
| | Tailwind CSS | 3.4.10 | 样式工具 |
